...
# mission duration = 15 years, call it 480 megaseconds
# go through in 500-kilosecond chunks

slot_width = 500000.0

for kilosec in range(960):
 date = d0 + (slot_width/86400.0)*kilosec
...
#  brightness_hist[brightness_bin] = 1+brightness_hist[brightness_bin]
 ...
 print kilosec*slot_width, num, too_near_sun, too_near_earth, too_faint, happy, count_ever_happy

 if (kilosec%160==159):
  print "Distance histogram at t=%s" % (slot_width*kilosec)
  ...
  # and the actual windowed output

  TEN=int(len(asteroids)*0.1)
  FIFTY=int(len(asteroids)*0.5)
  NINETY=int(len(asteroids)*0.9)
  NINETYFIVE=int(len(asteroids)*0.95)
 
  print "Window analysis at t=%s" % (slot_width*kilosec)
  for window_width in range(2,81):
   ...
   print window_width*slot_width, zz[TEN], zz[FIFTY], zz[NINETY], zz[NINETYFIVE]
